...
public class OfertaDao {
    ...
    public List<Oferta> getOfertasParaColaborar(String id_hab, String id_estudiante, String id_pet){
        try{
            return jdbcTemplate.query("SELECT * FROM oferta WHERE id_habilidad=? and id_estudiante!=? and estado!='colaborando' and id_oferta NOT IN(SELECT id_oferta FROM colaboracion WHERE id_pet = ?)", new OfertaRowMapper(), id_hab, id_estudiante, id_pet);
        }
        catch(EmptyResultDataAccessException e) {
            return new ArrayList<Oferta>();
        }
